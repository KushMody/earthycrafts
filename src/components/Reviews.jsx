...
const Reviews = () => {
    ...
    return (
        <div className={`w-full min-h-screen md:h-screen bg-[#080808] flex flex-col justify-start md:justify-center px-4 md:px-10 font-['Forum',serif] transition-opacity duration-1000 overflow-y-auto md:overflow-hidden scroll-smooth ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
            <style>{`
                .review-card {
            ...
            <div className={`max-w-full mx-auto ${isVisible ? 'is-visible' : ''} flex flex-col items-center h-full pt-32 md:pt-12`}>
                {/* Header - Compact */}
                <div className="text-center mb-10 md:mb-6 editorial-reveal" style={{ transitionDelay: '100ms' }}>
                    ...
                    <h1 className="text-[#efe7d2] text-[clamp(1.4rem,7vw,2.8rem)] leading-none uppercase tracking-tight px-4">
                        Voices of the <span className="text-[#c29d59]">Collection</span>
                    </h1>
                </div>

                {/* Gallery - Vertical stack on mobile, horizontal on desktop */}
                <div className="w-full flex list-none flex-col md:flex-row md:justify-center items-center md:items-stretch gap-0 md:gap-8 pb-12 md:pb-6 px-4 md:overflow-x-auto hide-scrollbar">
                    {reviews.map((review, index) => (
                        ...
                        >
                            {/* Top Portion: Image */}
                            <div className="h-[180px] md:h-[180px] w-full overflow-hidden relative">
                                <img 
                                    src={review.image} 
                                    alt={review.author} 
                            ...
                            <div className="flex-1 p-6 md:p-5 flex flex-col justify-between">
                                <div>
                                    <div className="flex text-[#c29d59] mb-4 md:mb-3 space-x-1">
                                        {[...Array(review.rating)].map((_, i) => (
                                            <span key={i} className="text-sm">★</span>
                                        ))}
                                    </div>
                                    <h4 className="text-[#efe7d2] text-xl md:text-lg uppercase tracking-wider mb-0.5">{review.author}</h4>
                                    <p className="text-[#c29d59]/60 text-xs md:text-[10px] uppercase tracking-[0.2em] mb-4 md:mb-3">{review.location}</p>
                                    <blockquote className="text-[#efe7d2] text-base md:text-lg leading-relaxed italic line-clamp-6 md:line-clamp-5">
                                        "{review.content}"
                                    </blockquote>
                                </div>
                                <div className="mt-5 md:mt-3 pt-4 md:pt-3 border-t border-white/5 flex justify-end">
                ...
                {/* Tiny CTA */}
                <div className="editorial-reveal mt-2 mb-4" style={{ transitionDelay: '700ms' }}>
                    <a 
                        href="/contact-us" 
                        className="text-[#c29d59] text-[10px] tracking-[0.3em] uppercase hover:text-[#efe7d2] transition-colors flex items-center gap-3 group"
                    >
                        <span>Begin Your Legacy</span>
                        <div className="h-[1px] w-5 bg-[#c29d59]/30 transition-all group-hover:w-10 group-hover:bg-[#efe7d2]"></div>
                    </a>
                </div>
            </div>
        </div>
