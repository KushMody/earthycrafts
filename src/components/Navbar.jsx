...
// Reusable custom SVG icons (Scaled up to 20x20 for better visibility)
const PhoneIcon = () => <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"></path></svg>;
const AtSignIcon = () => <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="4"></circle><path d="M16 8v5a3 3 0 0 0 6 0v-1a10 10 0 1 0-3.92 7.94"></path></svg>;

// Social icons (Scaled up to 26x26 for better touch targets and visual presence)
const WhatsappIcon = () => (
...
export const MobileMenu = ({ isOpen, onClose }) => {
  ...
  return (
    <div className={`fixed inset-0 bg-[#0c0c0c] z-[140] flex flex-col items-center justify-between py-12 transition-all duration-500 ease-in-out md:hidden ${isOpen ? 'translate-y-0 opacity-100' : 'translate-y-[-20px] opacity-0 pointer-events-none'}`}>
      ...
      <div className="flex-1 flex flex-col space-y-8 text-center justify-center items-center w-full">
        {mobileNavItems.map((item) => {
          ...
          return (
            <Link
              ...
              to={item.href}
              onClick={onClose}
              className={`group relative text-2xl tracking-[0.3em] font-['Forum',serif] uppercase transition-colors ${isActive ? 'text-white' : 'text-white/60 hover:text-white'
                }`}
            >
              {item.label}
              <span
                className={`absolute -bottom-2 left-1/2 -translate-x-1/2 h-[2px] bg-white transition-all duration-300 ${isActive ? 'w-full' : 'w-0 group-hover:w-full'
                  }`}
              ></span>
            </Link>
          );
        ...
        <Link to="/contact-us" className="nav-button mt-8" onClick={onClose}>Custom Inquiry</Link>
      </div>

      {/* REFINED Contact & Socials Area */}
      <div className={`w-full flex flex-col items-center space-y-8 pb-8 transition-all duration-700 delay-300 ${isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        ...
        {/* Details - Increased gap and text size */}
        <div className="flex flex-col items-start space-y-4 text-[#D9C394] font-['Forum',serif] text-base tracking-[0.1em]">
        ...
        {/* Social icons - Increased icon size and horizontal spacing */}
        <div className="flex items-center space-x-8 pt-2 text-[#D9C394]">
          <a href="#" target="_blank" rel="noreferrer" className="opacity-70"><WhatsappIcon /></a>
          <a href="#" target="_blank" rel="noreferrer" className="opacity-70"><FacebookIcon /></a>
          <a href="#" target="_blank" rel="noreferrer" className="opacity-70"><InstagramIcon /></a>
          <a href="#" target="_blank" rel="noreferrer" className="opacity-70"><LinkedinIcon /></a>
        </div>
      </div>
